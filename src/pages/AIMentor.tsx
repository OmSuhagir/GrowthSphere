...
const AIMentor = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const newAiMessage = { id: messages.length + 2, text: formattedResponse, sender: "ai" };
  ...
  const fetchGeminiResponse = async (query) => {
    try {
      ...
      return data?.candidates?.[0]?.content?.parts?.[0]?.text || "I'm here to assist you with your business queries!";
    } catch (error) {
              ...
              <CardContent className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                    ...
                    <div
                      className={`px-4 py-3 rounded-xl max-w-[80%] whitespace-pre-line text-sm ${msg.sender === "user" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-800"}`}
